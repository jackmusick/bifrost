"""
Huntress: Sync Organizations

Syncs Huntress organizations to Bifrost organizations and creates
...
async def sync_huntress_organizations() -> dict:
    ...
    try:
        huntress_organizations = await client.list_organizations()
    finally:
        await client.close()

    existing_mappings = {
        str(mapping.entity_id): mapping
        for mapping in (await integrations.list_mappings("Huntress") or [])
    }
    ...
    for organization in huntress_organizations:
    ...
    return {
        "total": len(huntress_organizations),
        "mapped": mapped,
