...
Syncs ConnectSecure companies to Bifrost organizations and creates
IntegrationMappings so org-scoped workflows can resolve the mapped company ID.

Entity model:
  entity_id   = ConnectSecure company id
...
async def sync_connectsecure_companies() -> dict:
    ...
    existing_mappings = {
        ...
        for mapping in (await integrations.list_mappings("ConnectSecure") or [])
    }
