"""
NinjaOne: Sync Organizations

Syncs NinjaOne organizations to Bifrost organizations and creates
IntegrationMappings so org-scoped workflows can resolve the mapped
organization ID.
"""
...
async def sync_ninjaone_organizations() -> dict:
    ...
    try:
        ninjaone_organizations = await client.list_organizations()
    finally:
        await client.close()

    existing_mappings = {
        str(mapping.entity_id): mapping
        for mapping in (await integrations.list_mappings("NinjaOne") or [])
    }
    ...
    for organization in ninjaone_organizations:
    ...
    return {
        "total": len(ninjaone_organizations),
        "mapped": mapped,
