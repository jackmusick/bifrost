...
def param(
    ...
    *,
    ...
    """
    DEPRECATED: Parameter decorator for workflow functions.

    This decorator is no longer needed - parameters are now automatically
    derived from function signatures. It remains for backward compatibility
    but does nothing.

    Use type hints and default values instead:

        @workflow
        async def my_workflow(
            name: str,               # Required parameter
            count: int = 1,          # Optional parameter with default
            enabled: bool = False,   # Optional boolean
        ):
            ...
    """
    import warnings
    warnings.warn(
        "@param decorator is deprecated. Parameters are now derived from function signatures.",
        DeprecationWarning,
        stacklevel=2,
    )
