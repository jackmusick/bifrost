...
async def list_components(
    ...
    is_draft: bool = Query(default=True, description="Get draft or live components"),
) -> AppComponentListResponse:
    """
    ...
    page = await get_page_or_404(ctx, app_id, page_id, is_draft)

    service = AppComponentsService(ctx.db)
    components = await service.list_components(page.id, is_draft)
...
async def get_component(
    ...
    ctx: Context = None,
    user: CurrentUser = None,
    is_draft: bool = Query(default=True, description="Get draft or live component"),
) -> AppComponentResponse:
    ...
    service = AppComponentsService(ctx.db)
    component = await service.get_component(page.id, component_id, is_draft)
...
async def create_component(
    ...
    service = AppComponentsService(ctx.db)
    existing = await service.get_component(page.id, data.component_id, is_draft=True)
    if existing:
        raise HTTPException(
