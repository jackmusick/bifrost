...
class TestPortableRefsPush:
    ...
    def test_app_push_has_portable_refs(
        ...
        test_app_with_workflow,
    ):
        """
        Verify app pushed to GitHub has portable refs, not UUIDs.

        1. App created with workflow_id (UUID)
        2. Push to GitHub
        3. Fetch .app.json from GitHub
        4. Verify workflow_id fields contain portable refs
        """
        app_info = test_app_with_workflow
        workflow = app_info["workflow"]

        # Execute sync with automatic conflict resolution
        job_result, job_id = execute_sync_with_auto_resolution(
        ...
        app_file_path = f"apps/{app_info['app']['slug']}.app.json"

        # Fetch file content from workspace (which is synced from GitHub)
        file_response = e2e_client.get(
            f"/api/files/editor/content?path={app_file_path}",
        ...
        if file_response.status_code == 200:
            ...
            # Parse the JSON to check for portable refs
            app_json = json.loads(content)

            # Check for _export metadata
            assert "_export" in app_json, "App JSON should have _export metadata"
            assert "workflow_refs" in app_json["_export"], "Should have workflow_refs list"

            # Check page's launch_workflow_id - should be portable ref
            if app_json.get("pages"):
                page = app_json["pages"][0]
                launch_ref = page.get("launch_workflow_id")
                if launch_ref:
                    assert is_portable_ref(launch_ref), (
                        f"launch_workflow_id should be portable ref, got: {launch_ref}"
                    )
                    assert workflow["file_path"] in launch_ref, (
                        f"Portable ref should contain file path: {launch_ref}"
                    )

            logger.info("App JSON has portable refs as expected")
        else:
            logger.warning(f"Could not fetch app file: {file_response.status_code}")
...
class TestPortableRefsPull:
    ...
    def test_app_pull_resolves_refs(
        ...
        """
        Verify app pulled from GitHub has UUIDs restored from portable refs.
        """
        ...
        create_remote_file(
            ...
            content=app_content,
            message="E2E test: App with portable refs",
        )
        logger.info(f"Created app file on GitHub: apps/{app_slug}.app.json")
        ...
        assert app, f"App {app_slug} not found after pull"

        # Fetch app definition to check workflow refs
        def_response = e2e_client.get(
            f"/api/applications/{app['id']}/draft",
            headers=platform_admin.headers,
        )
        if def_response.status_code == 200:
            definition = def_response.json().get("definition", {})
            pages = definition.get("pages", [])
            if pages:
                page = pages[0]
                launch_ref = page.get("launch_workflow_id")
                if launch_ref:
                    # Should be UUID now, not portable ref
                    assert is_uuid(launch_ref), (
                        f"launch_workflow_id should be UUID after pull, got: {launch_ref}"
                    )
                    assert launch_ref == workflow["id"], (
                        "launch_workflow_id should match workflow UUID"
                    )
            logger.info("App pulled with UUIDs restored")
